...
{
    public class GetCrawlJobDataBehaviour : DMSProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(DMSConstants.KeyName.ApiKey, nameof(DMSCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task DMSCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.DMS.Core.DMSCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
