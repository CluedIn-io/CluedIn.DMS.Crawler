...
namespace CluedIn.Provider.DMS
{
    public class DMSProvider : ProviderBase, IExtendedProviderMetadata
    {
        private readonly IDMSClientFactory _dmsClientFactory;

        public DMSProvider([NotNull] ApplicationContext appContext, IDMSClientFactory dmsClientFactory)
            : base(appContext, DMSConstants.CreateProviderMetadata())
        {
            _dmsClientFactory = dmsClientFactory;
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            if (jobData is DMSCrawlJobData dmsCrawlJobData)
            {
                ...
                dictionary.Add(DMSConstants.KeyName.ApiKey, dmsCrawlJobData.ApiKey);
            }
        ...
        public override async Task<AccountInformation> GetAccountInformation(ExecutionContext context, [NotNull] CrawlJobData jobData, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var client = _dmsClientFactory.CreateNew(dmsCrawlJobData);
        ...
        public string Icon => DMSConstants.IconResourceName;
        public string Domain { get; } = DMSConstants.Uri;
        public string About { get; } = DMSConstants.CrawlerDescription;
        public AuthMethods AuthMethods { get; } = DMSConstants.AuthMethods;
        public IEnumerable<Control> Properties => null;
        public string ServiceType { get; } = JsonConvert.SerializeObject(DMSConstants.ServiceType);
        public string Aliases { get; } = JsonConvert.SerializeObject(DMSConstants.Aliases);
        public Guide Guide { get; set; } = new Guide
        {
            Instructions = DMSConstants.Instructions,
            Value = new List<string> { DMSConstants.CrawlerDescription },
            Details = DMSConstants.Details

        };

        public string Details { get; set; } = DMSConstants.Details;
        public string Category { get; set; } = DMSConstants.Category;
        public new IntegrationType Type { get; set; } = DMSConstants.Type;
    }
}
